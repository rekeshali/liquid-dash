...
def make_tab(index: int, title: str | None = None, panels: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    return {
        "id": f"tab-{index}",
        "title": title or f"Canvas {index}",
        "panels": panels or [],
    }



def make_folder(index: int, tabs: list[dict[str, Any]], title: str | None = None, active_tab_id: str | None = None) -> dict[str, Any]:
    return {
        "id": f"folder-{index}",
...
def active_folder(state: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    return next((folder for folder in data.get("folders", []) if folder.get("id") == folder_id), None)
...
def active_tab(state: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    return next((tab for tab in folder.get("tabs", []) if tab.get("id") == tab_id), None)
...
def find_folder(state: dict[str, Any], folder_id: str | None) -> dict[str, Any] | None:
    if not folder_id:
        return None
    return next((folder for folder in state.get("folders", []) if folder.get("id") == folder_id), None)



def locate_tab(state: dict[str, Any], tab_id: str | None) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    if not tab_id:
        return None, None
...
def locate_panel(state: dict[str, Any], panel_id: str | None) -> tuple[dict[str, Any] | None, dict[str, Any] | None, dict[str, Any] | None]:
    if not panel_id:
...
def ensure_folder_has_tab(state: dict[str, Any], folder: dict[str, Any]) -> None:
    if folder.get("tabs"):
        if not folder.get("active_tab_id") or folder["active_tab_id"] not in {tab["id"] for tab in folder["tabs"]}:
...
def ensure_state_has_folder(state: dict[str, Any]) -> None:
    if state.get("folders"):
        if state.get("active_folder_id") not in {folder["id"] for folder in state["folders"]}:
...
def reduce_ui_event(
    ...
    if action == "panel.badge.remove":
        badge_index = int(payload.get("index", -1))
        if 0 <= badge_index < len(panel.get("badges", [])):
            panel["badges"].pop(badge_index)
        if editor_targets_panel(editor, panel["id"]) and editor.get("draft"):
            if 0 <= badge_index < len(editor["draft"].get("badges", [])):
                editor["draft"]["badges"].pop(badge_index)
        return state, editor

    if action == "panel.badge.cycle":
        badge_index = int(payload.get("index", -1))
        if 0 <= badge_index < len(panel.get("badges", [])):
            badge = panel["badges"][badge_index]
            badge["color"] = cycle_value(badge.get("color"), BADGE_COLORS)
        if editor_targets_panel(editor, panel["id"]) and editor.get("draft"):
            if 0 <= badge_index < len(editor["draft"].get("badges", [])):
                badge = editor["draft"]["badges"][badge_index]
                badge["color"] = cycle_value(badge.get("color"), BADGE_COLORS)
        return state, editor
...
def preview_for(panel: dict[str, Any]):
    ...
    return html.Div(
        [html.Div(className="proof-preview-dot", style=style) for style in dots],
...
def badge_row(panel: dict[str, Any]):
    ...
    return html.Div(
        [
            action_button(
                badge["label"],
                action="panel.badge.cycle",
                target=panel["id"],
                payload={"index": idx},
                bridge=UI_EVENT_BRIDGE,
...
def panel_summary(panel: dict[str, Any]) -> html.Ul:
    ...
    return html.Ul(
        [
            html.Li(f"Filter: {filter_state['variable']} {filter_state['operator']} {filter_state['value']}", className="proof-summary-item"),
        ...
        [
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            action_button("Lock", action="panel.lock.toggle", target=panel["id"], bridge=UI_EVENT_BRIDGE, className="proof-mini-btn" + (" is-active" if panel.get("locked") else "")),
                            action_button("Link", action="panel.link.toggle", target=panel["id"], bridge=UI_EVENT_BRIDGE, className="proof-mini-btn" + (" is-active" if panel.get("linked") else "")),
                            action_button("Settings", action="panel.settings.open", target=panel["id"], bridge=UI_EVENT_BRIDGE, className="proof-mini-btn"),
                            action_button("Duplicate", action="panel.duplicate", target=panel["id"], bridge=UI_EVENT_BRIDGE, className="proof-mini-btn"),
                            action_button("Delete", action="panel.delete", target=panel["id"], bridge=UI_EVENT_BRIDGE, className="proof-mini-btn proof-danger"),
                        ],
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            action_button("Add Badge", action="panel.badge.add", target=panel["id"], bridge=UI_EVENT_BRIDGE, className="proof-inline-btn"),
                            action_button(
                                "Remove Last",
                                action="panel.badge.remove",
                                target=panel["id"],
                                payload={"index": max(len(panel.get('badges', [])) - 1, 0)},
                                bridge=UI_EVENT_BRIDGE,
            ...
            html.Div([html.Div("Current Settings", className="proof-section-label"), panel_summary(panel)], className="proof-card-section"),
            ...
            html.Div(
                [
                    action_button(
                        ...
                        action="folder.activate",
                        target=folder["id"],
                        bridge=UI_EVENT_BRIDGE,
                        className=("proof-chip is-active" if folder["id"] == active_id else "proof-chip"),
                    ),
                    action_button("Rename", action="folder.rename.open", target=folder["id"], bridge=UI_EVENT_BRIDGE, className="proof-inline-btn"),
                    action_button("Delete", action="folder.delete", target=folder["id"], bridge=UI_EVENT_BRIDGE, className="proof-inline-btn proof-danger"),
                ],
        ...
        chips.append(
            html.Div(
                [
                    ...
                    action_button("Rename", action="tab.rename.open", target=tab["id"], bridge=UI_EVENT_BRIDGE, className="proof-inline-btn"),
                    action_button("Delete", action="tab.delete", target=tab["id"], bridge=UI_EVENT_BRIDGE, className="proof-inline-btn proof-danger"),
                ],
                className="proof-chip-group",
            )
        )
    ...
    if not badges:
        return html.Div(
            [
                ...
                action_button("Add badge", action="panel.badge.add", target=panel_id, bridge=UI_EVENT_BRIDGE, className="proof-inline-btn"),
            ],
        ...
        rows.append(
            html.Div(
                [
                    ...
                    action_button("Cycle", action="panel.badge.cycle", target=panel_id, payload={"index": idx}, bridge=UI_EVENT_BRIDGE, className="proof-inline-btn"),
                    action_button("Remove", action="panel.badge.remove", target=panel_id, payload={"index": idx}, bridge=UI_EVENT_BRIDGE, className="proof-inline-btn proof-danger"),
                ],
                className="proof-editor-badge-row",
            )
        )
    rows.append(action_button("Add badge", action="panel.badge.add", target=panel_id, bridge=UI_EVENT_BRIDGE, className="proof-inline-btn"))
...
def build_app() -> Dash:
    ...
    app.layout = StableRegion(
        id="proof-shell",
        region_name="proof-shell",
        className="proof-shell",
        children=[
            EventBridge(id=UI_EVENT_BRIDGE),
                ...
                [
                    html.Div(
                        [
                            ...
                            html.H1("Folders, tabs, and panels with a fixed callback graph", className="proof-title"),
                            html.P(
                                "Add, rename, delete, and switch between folders, tabs, and panels while keeping the callback graph small and predictable.",
                    ...
                    html.Div(
                        [
                            action_button("Add Folder", action="folder.add", bridge=UI_EVENT_BRIDGE, className="proof-toolbar-btn"),
                            action_button("Add Tab", action="tab.add", bridge=UI_EVENT_BRIDGE, className="proof-toolbar-btn"),
                            action_button("Add Time Series", action="panel.add", payload={"kind": "timeseries"}, bridge=UI_EVENT_BRIDGE, className="proof-toolbar-btn"),
                            action_button("Add Histogram", action="panel.add", payload={"kind": "histogram"}, bridge=UI_EVENT_BRIDGE, className="proof-toolbar-btn"),
                            action_button("Add Scatter", action="panel.add", payload={"kind": "scatter"}, bridge=UI_EVENT_BRIDGE, className="proof-toolbar-btn"),
                        ],
                    ...
                    DynamicRegion(id="folder-strip", region_name="folder-strip", bridge=UI_EVENT_BRIDGE, className="proof-strip"),
                    DynamicRegion(id="tab-strip", region_name="tab-strip", bridge=UI_EVENT_BRIDGE, className="proof-strip"),
                ],
                className="proof-header",
            ),
            html.Div(
                [
                    DynamicRegion(id="panel-grid", region_name="panel-grid", bridge=UI_EVENT_BRIDGE, className="proof-grid"),
                    StableRegion(
                        id="editor-shell",
                        region_name="shared-editor",
                                ...
                                [
                                    html.Div(
                                        [
                                            ...
                                            dcc.Dropdown(id="edit-kind", options=[{"label": KIND_META[k]["label"], "value": k} for k in PANEL_KINDS], clearable=False),
                                        ],
                                        className="proof-field",
                                    ),
                                    html.Div(
                                        [
                                            ...
                                            dcc.Dropdown(id="edit-mode", options=[{"label": m.title(), "value": m} for m in PANEL_MODES], clearable=False),
                                        ],
                                        className="proof-field",
                                    ),
                                ],
                                ...
                                [
                                    html.Div(
                                        [
                                            ...
                                            html.Div([html.Label("Variable", className="proof-label"), dcc.Dropdown(id="filter-variable", options=[{"label": x, "value": x} for x in ["altitude", "velocity", "temperature", "dynamic_pressure"]], clearable=False)], className="proof-field"),
                                            html.Div([html.Label("Operator", className="proof-label"), dcc.Dropdown(id="filter-operator", options=[{"label": x, "value": x} for x in [">", ">=", "<", "<=", "=="]], clearable=False)], className="proof-field"),
                                            html.Div([html.Label("Value", className="proof-label"), dcc.Input(id="filter-value", type="number", className="proof-input")], className="proof-field"),
                                            dcc.Checklist(id="filter-invert", options=[{"label": "Invert selection", "value": "invert"}], className="proof-checklist"),
                                        ],
                                        id="section-filter",
                                    ...
                                    html.Div(
                                        [
                                            ...
                                            html.Div(
                                                [
                                                    html.Div([html.Label("Line width", className="proof-label"), dcc.Input(id="line-width", type="number", className="proof-input"), html.Label("Line style", className="proof-label"), dcc.Dropdown(id="line-style", options=[{"label": x.title(), "value": x} for x in ["solid", "dash", "dot"]], clearable=False), dcc.Checklist(id="show-markers", options=[{"label": "Show markers", "value": "markers"}], className="proof-checklist"), html.Label("Smoothing", className="proof-label"), dcc.Dropdown(id="smoothing", options=[{"label": x.title(), "value": x} for x in ["off", "low", "medium", "high"]], clearable=False)], id="style-timeseries", className="proof-kind-subsection"),
                                                    html.Div([html.Label("Bins", className="proof-label"), dcc.Input(id="bins", type="number", className="proof-input"), dcc.Checklist(id="normalize", options=[{"label": "Normalize", "value": "normalize"}], className="proof-checklist"), dcc.Checklist(id="cumulative", options=[{"label": "Cumulative", "value": "cumulative"}], className="proof-checklist"), dcc.Checklist(id="reference-lines", options=[{"label": "Reference lines", "value": "reference_lines"}], className="proof-checklist")], id="style-histogram", className="proof-kind-subsection"),
                                                    html.Div([html.Label("Marker size", className="proof-label"), dcc.Input(id="marker-size", type="number", className="proof-input"), dcc.Checklist(id="trendline", options=[{"label": "Trendline", "value": "trendline"}], className="proof-checklist"), dcc.Checklist(id="density-overlay", options=[{"label": "Density overlay", "value": "density_overlay"}], className="proof-checklist"), html.Label("Palette", className="proof-label"), dcc.Dropdown(id="palette", options=[{"label": x.title(), "value": x} for x in ["blue", "sunset", "forest", "mono"]], clearable=False)], id="style-scatter", className="proof-kind-subsection"),
                                                ]
                                            ),
                                        ],
                                        id="section-style",
                                    ...
                                    html.Div(
                                        [
                                            ...
                                            html.Div([html.Div([html.Label("Rows", className="proof-label"), dcc.Input(id="subplot-rows", type="number", className="proof-input")], className="proof-field"), html.Div([html.Label("Cols", className="proof-label"), dcc.Input(id="subplot-cols", type="number", className="proof-input")], className="proof-field")], className="proof-two-col"),
                                            dcc.Checklist(id="share-x", options=[{"label": "Share X", "value": "share_x"}], className="proof-checklist"),
                                            dcc.Checklist(id="share-y", options=[{"label": "Share Y", "value": "share_y"}], className="proof-checklist"),
                                        ],
                                        id="section-subplot",
                                        className="proof-editor-section",
                                    ),
                                    html.Div([html.H3("Badges", className="proof-section-header"), html.Div(id="editor-badges")], id="section-badges", className="proof-editor-section"),
                                ],
                                className="proof-editor-body",
                            ),
                            html.Div([html.Button("Apply changes", id="editor-apply", className="proof-toolbar-btn")], className="proof-editor-footer"),
                        ],
                    ),
                ],
    ...
    def hydrate_shared_editor(editor_state):
        ...
        return (
            ...
            form["normalize"],
            form["cumulative"],
            form["reference_lines"],
            form["marker_size"],
            form["trendline"],
            form["density_overlay"],
            form["palette"],
            form["rows"],
            form["cols"],
            form["share_x"],
    ...
    @app.callback(
        ...
        State("edit-kind", "value"),
        State("edit-mode", "value"),
        State("filter-variable", "value"),
        State("filter-operator", "value"),
        State("filter-value", "value"),
        State("filter-invert", "value"),
        State("line-width", "value"),
        State("line-style", "value"),
        State("show-markers", "value"),
        State("smoothing", "value"),
        State("bins", "value"),
        State("normalize", "value"),
        State("cumulative", "value"),
        State("reference-lines", "value"),
        State("marker-size", "value"),
        State("trendline", "value"),
        State("density-overlay", "value"),
        State("palette", "value"),
        State("subplot-rows", "value"),
        State("subplot-cols", "value"),
        State("share-x", "value"),
        State("share-y", "value"),
        State(CANVAS_STORE, "data"),
        State(EDITOR_STORE, "data"),
